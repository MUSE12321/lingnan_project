...
public class LivingServiceImpl implements LivingService{
	...
	@Override
	public Page<Device> query(LivingQuery livingQuery) {
		
		livingQuery.init();
		
		Page<Device> page= new Page<Device>();
		
		String SQL = "select * from device where location='客厅'"+livingQuery.getWhereSQL();
		
		List<Object> params = livingQuery.getWhereParams();//获取查询参数
		...
		//当前查询条件下的记录数
		int count = findCount("select count(*) from device where location='客厅' "+livingQuery.getWhereSQL(),livingQuery.getWhereParams());
